    ...
    return (
        <div className="space-y-6 animate-fade-in p-4 bg-gray-50 min-h-screen font-sans relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
                ...
                <div className="bg-[#2d3748] rounded-none p-3 text-white flex gap-3 items-center shadow-sm">
                    <div className="bg-white/10 p-2 rounded-none"><Users className="w-6 h-6" /></div>
                    <div>
                ...
                {/* Male - Green */}
                <div className="bg-[#48bb78] rounded-none p-3 text-white flex gap-3 items-center shadow-sm">
                    <div className="bg-white/10 p-2 rounded-none"><User className="w-6 h-6" /></div>
                    <div>
                ...
                {/* Female - Pink */}
                <div className="bg-[#f687b3] rounded-none p-3 text-white flex gap-3 items-center shadow-sm">
                    <div className="bg-white/10 p-2 rounded-none"><User className="w-6 h-6" /></div>
                    <div>
                ...
                {/* Members - Teal */}
                <div className="bg-[#38b2ac] rounded-none p-3 text-white flex gap-3 items-center shadow-sm">
                    <div className="bg-white/10 p-2 rounded-none"><CreditCard className="w-6 h-6" /></div>
                    <div>
                ...
                {/* Non-Members - Gray */}
                <div className="bg-[#4a5568] rounded-none p-3 text-white flex gap-3 items-center shadow-sm">
                    <div className="bg-white/10 p-2 rounded-none"><UserX className="w-6 h-6" /></div>
                    <div>
                        <h3 className="text-2xl font-bold leading-none">{clients.filter(c => c.type !== 'Member').length}</h3>
                ...
                {/* New Clients - Orange */}
                <div className="bg-[#f6ad55] rounded-none p-3 text-white flex gap-3 items-center shadow-sm">
                    <div className="bg-white/10 p-2 rounded-none"><Clock className="w-6 h-6" /></div>
                    <div>
                        <h3 className="text-2xl font-bold leading-none">{clients.length}</h3>
            ...
            {/* Toolbar */}
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-2 rounded-none shadow-sm border border-gray-200 relative z-10">
                <div className="flex items-center w-full md:w-1/2">
                    <button className="bg-white border border-gray-300 px-3 py-2 rounded-none hover:bg-gray-50">
                        <Search className="w-4 h-4 text-blue-500" />
                    </button>
                    <input
                        className="w-full border-t border-b border-r border-gray-300 rounded-none px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
                        placeholder="Search.."
                        value={searchTerm}
                ...
                <div className="flex items-center gap-4 relative">
                    <span className="text-sm font-medium text-gray-600 bg-gray-100 px-3 py-1 rounded-none">
                        {dateRange.from} - {dateRange.to}
                    </span>
                    <Button
                        onClick={() => setIsCalendarOpen(!isCalendarOpen)}
                        className="bg-[#4a5568] hover:bg-gray-700 text-white text-xs font-bold px-4 py-2 h-9 flex items-center gap-2 rounded-none"
                    >
                    ...
                    {isCalendarOpen && (
                        <div className="absolute top-12 right-0 bg-white shadow-xl rounded-none border border-gray-200 p-4 w-72 z-50 animate-in fade-in zoom-in-95">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <input
                                        type="date"
                                        className="w-full border rounded-none px-2 py-1 text-sm focus:border-blue-500 outline-none"
                                ...
                                <div className="flex gap-2 pt-2">
                                    ...
                                    <Button
                                        size="sm"
                                        className="flex-1 bg-blue-600 hover:bg-blue-700 h-8 text-xs rounded-none"
                                        onClick={() => setIsCalendarOpen(false)}
                                    >
                                        Apply
                                    </Button>
                            ...
                            {!loading && filteredClients.map((client, idx) => (
                                <tr key={client.id} className="hover:bg-blue-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        <span className="bg-[#3182ce] text-white text-[10px] font-bold px-2 py-0.5 rounded-none shadow-sm">
                                            {client.gender}
                                        </span>
                                    </td>
